...
// RECEIVE Stock (Bulk) - Used by the new Factura-style screen
app.post('/api/products/receive-bulk', async (req, res) => {
    const { items, global_proveedor_id, factura_nro } = req.body;
    
    if (!Array.isArray(items) || items.length === 0) {
        return res.status(400).json({ success: false, message: 'No hay productos para recibir' });
    }
    ...
    try {
        ...
        for (const item of items) {
            ...
            // Si el item no tiene proveedor_id, usa el global
            const itemProveedorId = proveedor_id || global_proveedor_id || null;
            ...
            await client.query(
                ...
                actualizado_en = NOW() 
                WHERE id = $6`,
                [cantidad, costo_usd, margen_ganancia, aplica_iva, itemProveedorId, id]
            );
            ...
            await client.query(
                `INSERT INTO historial_compras (producto_id, proveedor_id, cantidad, costo_unitario_usd) 
                 VALUES ($1, $2, $3, $4)`,
                [id, itemProveedorId, cantidad, costo_usd]
            );

            // Opcional: si la factura_nro está presente, podríamos guardarlo en una tabla de facturas en el futuro.
            
            // Audit Log (No podemos usar global.logAudit fácilmente con un cliente de transacción si no recibe `client`, 
            // así que lo insertamos directamente para mantener transaccionalidad)
            await client.query(
                `INSERT INTO audit_logs (usuario_id, action, target_table, target_id, details, ip_address) 
                 VALUES ($1, $2, $3, $4, $5, $6)`,
                [req.headers['x-user-id'] || null, 'RECEIVE_STOCK_BULK', 'productos', id, JSON.stringify({ quantity: cantidad, cost: costo_usd, dest: destino, factura: factura_nro }), req.ip]
            );
        }

        await client.query('COMMIT');
        res.json({ success: true, message: 'Recepción procesada exitosamente' });
    } catch (err) {
        await client.query('ROLLBACK');
        console.error('Error en receive-bulk:', err);
        res.status(500).json({ success: false, message: 'Error procesando la recepción', error: err.message });
    } finally {
        client.release();
    }
});
...
app.post('/api/products/bulk-create', async (req, res) => {
    ...
    try {
        ...
        for (const p of products) {
            try {
                const pCodigo = p.codigo || p.CODIGO;
                const pNombre = p.nombre || p.NOMBRE;
                const pCategoria = p.categoria || p.CATEGORIA;
                const pProveedor = p.proveedor || p.PROVEEDOR;
                const pCosto = p.costo || p.COSTO_USD || p.COSTO;
                const pMargen = p.margen || p.MARGEN;
                const pMinimo = p.minimo || p.STOCK || p.STOCK_MINIMO || p.MINIMO || 5;
                const pMarca = p.marca || p.MARCA;

                // 1. Resolve Category
                let catId = null;
                const catName = pCategoria && typeof pCategoria === 'string' && pCategoria.trim() ? pCategoria.trim() : 'General';
                ...
                let provId = null;
                const provName = pProveedor && typeof pProveedor === 'string' && pProveedor.trim() ? pProveedor.trim() : 'General';
                ...
                if (!pCodigo) {
                    results.failed++;
                    results.errors.push(`Falta el código del producto`);
                    continue;
                }
                
                const check = await client.query('SELECT id FROM productos WHERE codigo = $1', [pCodigo]);
                if (check.rows.length > 0) {
                    results.failed++;
                    results.errors.push(`Código duplicado: ${pCodigo}`);
                    continue; // Skip
                }
                ...
                const pCodigoFallback = p.codigo || p.CODIGO || 'fila';
                results.failed++;
                results.errors.push(`Error en ${pCodigoFallback}: ${rowErr.message}`);
            }
        }
    ...
    try {
        ...
        for (const item of items) {
            try {
                ...
                const itemCantidad = item.cantidad || item.CANTIDAD;
                const itemCosto = item.costo || item.COSTO_NUEVO || item.COSTO;
                const itemProveedor = item.proveedor || item.PROVEEDOR;
                const itemDestino = item.destino || item.DESTINO || 'venta';
                ...
                const itemCodigoFallback = item.codigo || item.CODIGO || 'fila';
                results.failed++;
